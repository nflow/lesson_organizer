...
func (h *Handler) RetrieveBoardGoals(w http.ResponseWriter, r *http.Request) {
	...
	goals := &[]model.BoardGoal{}
	if err := h.DB.Order("rank").Preload(clause.Associations).Find(goals, "board_id = ?", boardId).Error; err != nil {
		RespondWithError(w, http.StatusInternalServerError, err)
	}

	RespondWithSuccess(w, goals)
}

func (h *Handler) CreateGoalInBoard(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	createGoalDto := &model.CreateGoal{}

	if !HandleBodyDecode(w, r, createGoalDto) {
		return
	}
	...
	var lastGoal = &model.BoardGoal{}
	if err := h.DB.Where("board_id = ?", boardId).Order("rank desc").FirstOrInit(lastGoal, &model.BoardGoal{Rank: 0}).Error; err != nil {
		RespondWithError(w, http.StatusInternalServerError, err)
		return
	}

	newBoardGoal := &model.BoardGoal{ID: uuid.New(), BoardID: boardId, Text: createGoalDto.Text, Rank: lastGoal.Rank + 100}
	if err := h.DB.Model(&model.Board{ID: boardId}).Association("Goals").Append(newBoardGoal); errors.Is(err, gorm.ErrRecordNotFound) {
		RespondWithError(w, http.StatusInternalServerError, err)
		return
	}
...
func (h *Handler) RemoveGoalFromBoard(w http.ResponseWriter, r *http.Request) {
	...
	var goal model.BoardGoal
	if err := h.DB.First(&goal, "id = ? AND board_id = ?", goalId, boardId).Error; err != nil {
		RespondEmptyWithCode(w, http.StatusNotFound)
		return
	} else if err != nil {
		RespondWithError(w, http.StatusInternalServerError, err)
